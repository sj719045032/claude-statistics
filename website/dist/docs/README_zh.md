...
一款原生 macOS 菜单栏应用，用于实时查看 [Claude Code](https://docs.anthropic.com/en/docs/claude-code)、[Codex CLI](https://github.com/openai/codex)、Gemini CLI 以及插件市场安装的 AI 工作流插件：会话、订阅用量、账号、Token 与费用统计。

## 当前亮点

- **刘海岛（Notch Island）** — 停驻在 MacBook 刘海区的实时活动面板，展示所有在跑的 Claude Code / Codex / Gemini 会话。权限审批卡片（Allow/Deny）、等待输入提示、一键跳回会话所在的精确终端 tab。
- **插件市场** — 在 Settings → Plugins → Discover 里安装、更新、禁用或卸载 `.csplugin`。官方插件目录维护在 [`claude-statistics-plugins`](https://github.com/sj719045032/claude-statistics-plugins)。
...
- **订阅扩展插件** — GLM Coding Plan 等第三方 endpoint 可以通过插件加入 token-based 订阅身份，与原生 OAuth/JWT 账号并列管理。
...
Claude Statistics 内置插件市场，用于安装 provider 适配、终端/编辑器集成、分享卡扩展和订阅 endpoint。

- 打开 **Settings → Plugins → Discover** 拉取官方目录。
- 点击 **Install** 会下载 `.csplugin.zip`、校验 SHA-256、解压、记录信任并热加载，无需重启。
- 已安装的用户插件可在 **Settings → Plugins → Installed** 禁用、更新或卸载。
- 当前目录分类为 `provider`、`terminal`（界面显示为 Integrations/集成）、`share-card`、`subscription`、`utility`。
- 旧分类仍兼容：`vendor` 会归到 `provider`；`chat-app` / `editor-integration` 会归到 `terminal`。

公开插件目录在 [`sj719045032/claude-statistics-plugins`](https://github.com/sj719045032/claude-statistics-plugins)。插件作者可以把 `.csplugin.zip` 发布到 GitHub Releases，然后向目录提交包含 `downloadURL`、`sha256`、`version` 和权限声明的 PR。
...
- Codex：从本地 JWT 解码身份和套餐信息，并结合 transcript 重建本地用量
- Gemini：按 Pro / Flash / Flash Lite 分组展示额度、重置时间和本地 Token 趋势图
- Marketplace 订阅扩展：GLM Coding Plan 等第三方 endpoint 可通过 token-based 账号和 quota 接入
...
底栏的切换器可随时在不同 Provider 间切换。Claude 是默认 chassis provider；Codex、Gemini 和未来 Provider 可以通过插件交付。
...
- **Marketplace Provider** — provider 插件可以贡献 descriptor、账号 UI、定价、hooks、状态行集成和 transcript/session 解析能力
...
- 字体缩放控制
- 自定义 tab 排序
- 插件市场：Discover / Installed、安装/更新/禁用/卸载、信任重置和目录刷新
- 模型定价管理（查看、编辑、抓取最新定价）
...
**插件系统**
- 从官方 marketplace catalog、App bundle 或用户插件目录加载 `.csplugin`
- Marketplace 下载会经过 SHA-256 校验和 manifest ID 匹配，确认后热加载
- provider、终端/编辑器、分享卡和订阅插件通过 `ClaudeStatisticsKit` 注册能力
- 解析和统计仍在本机完成；只有打开或刷新 Discover 时才拉取 catalog metadata
...
├── Plugins/                # ClaudeStatisticsKit SDK + 一方插件 targets
...
#   build/marketplace/*.csplugin.zip           — marketplace 插件包
```
...
6. 打包 marketplace `.csplugin.zip`，同时生成对应 `.sha256`
7. 在结尾打印一条现成的 `gh release create` 命令，包含 DMG、完整 ZIP、
   本次生成的所有 `.delta` 文件以及插件包
...
| 插件 | 浏览 Discover、安装/更新插件、禁用/卸载用户插件、重置信任决策 |
| 模型定价 | 查看、编辑或抓取最新模型定价 |
