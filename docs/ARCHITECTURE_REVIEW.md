...
>
> **实施进度**：
> - ✅ 3.1 AppState 拆分（实用版：抽出 4 个协作者 — `AccountManagers` /
>   `ProviderContextRegistry` / `UsageVMRegistry` / `NotchRuntimeCoordinator`，
>   AppState 由 358 → 230 行，未走原计划的 4-ObservableObject 路线）
...
### 3.1 AppState God Object 拆分 ✅ 已完成（实用版）

> 落地说明：未采用原计划的"4 个 ObservableObject + Coordinator"方案——
> 那条路要求所有 6 个观察 store 的 View 都加 `@EnvironmentObject`，
> 改动面过大且 ObservableObject 不必要（多数被抽对象不需要广播变更）。
>
> 实际抽出的 4 个协作者全部是值/类，没有用 ObservableObject：
> - `AccountManagers`（struct，4 个 account manager + 按 kind reload）
> - `ProviderContextRegistry`（class，per-kind store + sessionVM + 运行时桥订阅 + bootstrap/ensure/rebuild）
> - `UsageVMRegistry`（class，primary + 多个 secondary VM + 切换 swap）
> - `NotchRuntimeCoordinator`（class，notch runtime 的 purge/refresh/restore 委派）
>
> 结果：AppState 由 358 → 230 行；类型系统拦住的依赖关系更明确；
> 切换/重建逻辑在各自文件里独立可读。
> `appState.usageViewModel` 等保留为转发属性，全部消费方零改动。
> `UsageVMRegistry` 增加 5 个单元测试覆盖路由 + swap 逻辑。
