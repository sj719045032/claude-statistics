...
一款原生 macOS 菜单栏应用，用于实时查看 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 和 [Codex CLI](https://github.com/openai/codex) 的会话、订阅用量以及 Token / 费用统计。

## v2.3.0 亮点

- **多 Provider 支持** — 通过底栏切换器在 Claude Code 与 Codex CLI 之间随时切换
- Provider 感知 UI：Tab 栏、账号卡片、状态行集成均随当前 Provider 自动适配
- 安装检测：未安装的 Provider 在切换器中自动置灰禁用
- Codex 用户信息本地从 JWT 解码，无需额外 API 请求
- 切换 Provider 时账号卡片显示全卡 loading，不再出现"?"头像闪烁
...
Claude Statistics 会自动发现并解析会话数据：Claude Code 来自 `~/.claude/projects/`，Codex CLI 来自 `~/.codex/projects/`。通过底栏切换器选择当前 Provider。
...
Claude Statistics 支持两个 Provider，各自使用本地优先的数据来源：
...
- Session 扫描和 transcript 解析适配 Codex 文件格式
...
├── Providers/              # SessionProvider 协议 + Claude 和 Codex 实现
